...
public class Hatred2Power: CustomPowerModel
{
    ...
    {
        if (!Owner.IsMonster || Owner.Monster==null || !creature.IsPlayer || creature.Player == null || creature.Player.Creature.Name != TargetPlayer)
        {
            return Task.CompletedTask;
        }
        List<string> filteredNames = CombatState.Players
            .Where(player => player.Creature.IsAlive)      // 1. 判断条件：满足条件的保留，不满足的跳过
            .Select(player => player.Creature.Name)         // 2. 提取值：获取满足条件对象的某个值
            .ToList();
